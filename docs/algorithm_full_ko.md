# TFSD8‑Neuron‑Quant 알고리즘 (v0.1.5)
...
TFSD8‑Neuron‑Quant는 이 원리를 디지털로 옮겨, **저전력 이벤트 기반 인코딩**을 구현합니다.
...
- **TFSD8(2^E)**: 배율이 2의 거듭제곱 → **곱셈기가 아닌 시프트**로 구현(전력/면적/지연에 유리).  
...
- 개요/시간/오차피드백/스케일 흐름
   ![TFSD Master overview](diagrams/TFSD8_MasterOverview.svg) 
- 인코더‑디코더 동기/토큰 스트림
   ![TFSD8_sync_auto](diagrams/ue8m0_sync_auto_vertical.svg)
